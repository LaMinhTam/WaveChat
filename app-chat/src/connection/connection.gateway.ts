  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  @SubscribeMessage('end-call-request')
  async handleEndCallVideo(
    @ConnectedSocket() client: SocketWithUser,
    @MessageBody() data: SendCallRequestDto,
  ) {
    try {
      ...
      this.server.to(data.target_user_id.toString()).emit('end-call-video', {
        user_from: client.user,
        user_to: data.target_user_id,
        message: data?.message || 'Tắt call!',
      });
    } catch (error) {
      console.log('error:', error);
      this.emitSocketError(
