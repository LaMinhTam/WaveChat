...
export class ConversationGroupController {
  ...
  @Get('waiting-member')
  @ApiOperation({
    summary: 'Lấy thành viên đang chờ phê duyệt cuộc trò chuyện ',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Lấy thành viên đang chờ phê duyệt cuộc trò chuyện',
  })
  async getWaitingMembersConversation(
    ...
    const data: any =
      await this.conversationGroupService.getWaitingMembersConversation(
        param.conversation_id,
        req.user._id,
      );
    return data;
  ...
  @Post('confirm-member')
  @ApiOperation({ summary: 'Phê duyệt thành viên cuộc trò chuyện ' })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Phê duyệt thành viên cuộc trò chuyện ',
  })
  async confirmMembersConversation(
    @Request() req: RequestWithUser,
    @Query() param: DetailConversation,
    @Body() body: ConfirmMemberConversationDto,
  ) {
    const data: any =
      await this.conversationGroupService.confirmMembersConversation(
        param.conversation_id,
        req.user,
        body,
      );
